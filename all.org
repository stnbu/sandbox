...
- Your wallet as your crypto keypair
...
- Why can't blockchain project A store some checkpointing information on blockchain project B? For example, if I could get (forgets name) the data necessary to bootstrap an Ethereum node (without starting from genisis, is the point) from both Bitcoin and Solana, and then only have to calculate the last 20 blocks... can't I just forget about all the history before that? Can't we just chain these chains together for added safety, with no judgement? For selfish reasons?
